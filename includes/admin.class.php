...
public function fetchClientBookingDetails($clientid){
	...
	public function getBlockRoomDetails(){
		...
		if(mysql_num_rows($result)){
			while($row=mysql_fetch_assoc($result)){
				$room_res=mysql_query("select room_id from bsi_reservation  where `bookings_id`='".$row['booking_id']."'");
				$count=mysql_num_rows($room_res);
				if($count){
					 $row77=mysql_fetch_assoc($room_res);
					 $qry_res=mysql_query("select * from bsi_room where room_ID='".$row77['room_id']."'");
					 if(mysql_num_rows($qry_res)){
						 $row99=mysql_fetch_assoc($qry_res);
						 $cap_title=mysql_fetch_assoc(mysql_query("select title from bsi_capacity where id='".$row99['capacity_id']."'"));
						 $type_title=mysql_fetch_assoc(mysql_query("select type_name from bsi_roomtype where roomtype_ID='".$row99['roomtype_id']."'"));
						 $getHtml.='<tr><td>'.$row['block_name'].'</td><td>'.$row['StartDate']."-".$row['EndDate'].'</td><td>'.$type_title['type_name']."(".$cap_title['title'].')</td><td>'.$count.'</td><td align="right"><a href="'.$_SERVER['PHP_SELF'].'?action=unblock&bid='.$row['booking_id'].'&rti='.$row99['roomtype_id'].'&cid='.$row99['capacity_id'].'">'.UN_BLOCK.'</a></td></tr>';
					 }
				}
...
<td>'.$bsiCore->config['conf_currency_symbol'].'<input type="text" name="sat" id="sat" class="required number" value="'.$row['sat'].'" size="4" /></td>
	...
	public function changePassword(){
		...
		$result  = mysql_query("select * from bsi_admin where pass=\"" . ($oldpass) . "\" and id=".$adminid);
		if(@mysql_num_rows($result)){
			mysql_query("update bsi_admin set pass='".($newpass)."' where id=".$adminid);
